// ============================================================
// Restore data dari file backup JSON -> upsert ke Supabase (ABB Star Reward).
// Pakai untuk memulihkan data yang terhapus/terubah (project yang SAMA).
//
// Contoh:
//   node ops/backup/restore.mjs ops/backup/dumps/supabase_backup_2026-07-21_02-00-00.json
...
const cfg = JSON.parse(
  readFileSync(join(__dirname, "backup.config.json"), "utf8")
);

const file = process.argv[2];
if (!file || !existsSync(file)) {
  console.error(
    "Usage: node restore.mjs <path-backup.json> [--only=tabel1,tabel2] [--dry-run]"
  );
  process.exit(1);
}
...
const order = payload.meta?.tableOrder || ["profiles", "stores", "visits"];
...
console.log(
  `Dibuat: ${payload.meta?.createdAt || "?"}${
    dryRun ? "  (DRY-RUN — tidak menulis)" : ""
  }\n`
);
...
for (const t of order) {
  ...
  if (!rows.length) {
    console.log(`- ${t}: 0 baris (lewati)`);
    continue;
  }
  if (dryRun) {
    console.log(`- ${t}: akan upsert ${rows.length} baris`);
    continue;
  }
